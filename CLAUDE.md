...
None detected. This role contains no credential or key variables.

### Commit scopes

Role-specific subsystem scopes: `debian`, `redhat`, `packages`

### Settled decisions

`robertdebock.rsyslog` is a runtime dependency, not a `meta/dependencies`
entry. Adding it to `meta/dependencies` would cause double-execution (it
runs once automatically then again via `import_role` in the task files)
and would bypass the `rsyslog_receiver` guard, applying rsyslog config to
receiver hosts. The correct consumer-facing contract is `requirements.yml`.

`rsyslog_receiver: false` lives in `defaults/main.yml`. It is part of the
role's public interface. Leaving it undefined would cause tasks to fail on
any host that does not explicitly set it in inventory.

Legacy `/etc/rsyslog.d/` cleanup runs unconditionally — even on receiver
hosts. This is intentional: the fragments being removed (`01-gnutls.conf`,
`90-remote-forward.conf`, `forwarding.conf`) are client-side artefacts
that should not exist on any host regardless of receiver status.

`tasks/preflight.yml` is imported with `ansible.builtin.import_tasks`
(static), not `include_tasks` (dynamic). This guarantees the assertions
run under all tag-filtered invocations and allows ansible-lint to analyse
the tasks at parse time.

OS-specific logic is split into `tasks/debian.yml` and `tasks/redhat.yml`
rather than guarded inline. This keeps apt and dnf specifics isolated and
makes each file independently readable.

### Open questions

If a task touches one of these, leave a `# TODO(open-q):` comment:

Should legacy `/etc/rsyslog.d/` cleanup also run on RedHat family hosts?
Currently `tasks/redhat.yml` has no cleanup step — only `tasks/debian.yml`
does. If the same legacy fragments can appear on EL deployments this is a
bug.

Should `robertdebock.rsyslog` be pinned to a specific version in
`requirements.yml`? No `requirements.yml` exists yet. An unpinned install
will pull the latest tag, which may introduce breaking upstream changes.

Does `robertdebock.rsyslog` require per-distro variable overrides? The
`vars/` directory is empty. If the upstream role behaves differently across
Debian/EL without explicit var overrides, OS-specific files will be needed.
...
1. ~~Fix `meta/argument_specs.yml`~~ — done.
2. ~~Update `README.md`~~ — done.
3. Add `requirements.yml` — declare `robertdebock.rsyslog` as an explicit
   install-time dependency so consumers can run
   `ansible-galaxy role install -r requirements.yml`.
4. Create `DESIGN.md` — document why `robertdebock.rsyslog` is the
   backend, what the legacy cleanup step removes and why, when
...
Install the upstream dependency before using this role:

    ansible-galaxy role install -r requirements.yml

All rsyslog configuration (forwarding targets, TLS settings, queue
parameters, rulesets) is controlled through `robertdebock.rsyslog`
variables. Set those variables in the same play or group_vars — they are
visible to the imported role through normal Ansible variable scoping.

Set `rsyslog_receiver: true` in inventory for any host that should receive
logs rather than forward them. The role will skip package installation and
the robertdebock.rsyslog delegation entirely on those hosts. Legacy
`/etc/rsyslog.d/` fragment cleanup still runs on receiver hosts.
